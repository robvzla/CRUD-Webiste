...
<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST")
    {
        ...
        */
        ...
        {
            header("location: ../updateDetails.php?error=emptyinput");
            exit();
        }

        if (arrayInput($temperature, $breakfast, $lunch, $activities) !== false) 
        {
            header("location: ../updateDetails.php?error=invalidinput");
            exit();
        }
        ...
        //  After all validations are done and succesfully passed, we update the child daily details in the database
        updateDailyDetail($db_connection, $temperature, $breakfast, $lunch, $activities, $_SESSION["idkid"], $_SESSION["date"]);
    }
    else
    {
